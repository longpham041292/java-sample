...
public class CreditETicketBookingResponse {
	...
	public CreditETicketBookingResponse(CreditBookingEntity bookingEntity) {
		this.bookingDay = ReportUtils.convertDateToEpochMiliSecond(bookingEntity.getBookingDay());;
		...
		this.startTime = ReportUtils.convertDateTimeToEpochMiliSecond(bookingEntity.getStartTime());
		this.endTime = ReportUtils.convertDateTimeToEpochMiliSecond(bookingEntity.getEndTime());
		this.confirmationCode = bookingEntity.getConfirmationCode();
		this.studioCover = bookingEntity.getStudioCover();
		
		OpeningHour openingHour = (OpeningHour)ReportUtils.toObjectList(bookingEntity.getEtickets().get(0).getOpeningHours(), OpeningHour.class);
		if(openingHour != null) {
