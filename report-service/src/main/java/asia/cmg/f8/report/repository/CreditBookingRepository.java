...
public interface CreditBookingRepository extends JpaRepository<CreditBookingEntity, Long> {
	...
	@Query(value =
			...
			"WHERE cb.status = ?1 AND csb.end_time <= NOW()", nativeQuery = true)
	...
	@Query(value = "SELECT new asia.cmg.f8.report.dto.BookingHistoryDTO(c) FROM CreditBookingEntity c WHERE clientUuid = ?1 AND createdDate BETWEEN ?2 and ?3 AND c.status in ?4 AND c.bookingType in ?5",
	...
	@Query(value = "SELECT new asia.cmg.f8.report.dto.BookingHistoryDTO(c)  FROM CreditBookingEntity c WHERE clientUuid = ?1 AND createdDate BETWEEN ?2 and ?3 AND c.status in ?4 AND c.bookingType in ?5 AND studioName LIKE %?6% ",
	...
	@Query(value = "SELECT new asia.cmg.f8.report.dto.SpendingStatisticDTO("
			+ "cbe.createdDate, "
			+ "cbe.creditAmount) "
	...
	List<BookingServiceDTO> eticketBookingManagement(
