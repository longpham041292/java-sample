...
import asia.cmg.f8.report.entity.database.SessionEntity;

@Repository
public interface SessionReportRepository extends JpaRepository<SessionEntity, Long> {

	String exportSessionQueryFilterByCheckingClubUuid = "SELECT new asia.cmg.f8.report.dto.SessionExportDTO(ss.id, ss.startTime, ss.endTime, ss.status," + 
	...
	String exportSessionQuery = "SELECT new asia.cmg.f8.report.dto.SessionExportDTO(ss.id, ss.startTime, ss.endTime, ss.status," + 
																					"eu.uuid, eu.fullName, pt.fullName, pt.level," + 
																					"ss.bookingClubName, ss.bookingClubAddress," + 
																					"ss.checkinClubName, ss.checkinClubAddress, ss.checkinDate) " + 
								"FROM SessionEntity ss " + 
