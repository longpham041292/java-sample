...
@Entity
@Table(name = "credit_session_bookings", uniqueConstraints = @UniqueConstraint(name = "uuid_UNI", columnNames = {
		"uuid" }))
public class CreditSessionBookingEntity{
	...
	private LocalDateTime startTime; // the current start time. It's equal with start time of the current linked
										// event.
...
//	}
	...
	public String toString() {
		return String.format("{userUuid: %s, ptUuid: %s, bookedBy: %s, creditAmount: %s, startTime: %s, endTime: %s}",
				userUuid, ptUuid, bookedBy, creditAmount, startTime, endTime);
	}
}
