...
public class LeepEventService {
	...
	public LeepEventService(LeepWebConfiguration leepWebConfig) {
	...
	public List<LeepContentEntity> getTopLeepEvents() {
		try {
			String query = "SELECT * FROM leep_events WHERE published = 1 ORDER BY ordering DESC LIMIT 5";
			List<LeepContentEntity> events = new ArrayList<LeepContentEntity>();
