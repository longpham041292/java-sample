...
public interface UserOutcomeRepository extends JpaRepository<UserOutcomeEntity, Long> {
	...
	@Query(value = "SELECT su.full_name, su.username, su.email, su.phone, SUM(uo.total_package), SUM(uo.credit_amount) FROM user_outcome uo JOIN session_users su ON uo.owner_uuid = su.uuid WHERE uo.to_date BETWEEN ?1 AND ?2 GROUP BY uo.owner_uuid \n#pageable\n", 
			countQuery = "SELECT COUNT(*) FROM (SELECT su.full_name, su.username, su.email, su.phone, SUM(uo.total_package), SUM(uo.credit_amount) FROM user_outcome uo JOIN session_users su ON uo.owner_uuid = su.uuid WHERE uo.to_date BETWEEN ?1 AND ?2 GROUP BY uo.owner_uuid) c", nativeQuery = true)
	Page<Object[]> getTrainerProfitReport(final LocalDateTime startTime, final LocalDateTime endTime,
			final Pageable pageable);

	@Query(value = "SELECT su.full_name, su.username, su.email, su.phone, SUM(uo.total_package), SUM(uo.credit_amount) FROM user_outcome uo JOIN session_users su ON uo.owner_uuid = su.uuid WHERE uo.to_date BETWEEN ?1 AND ?2 AND su.full_name LIKE %?3% GROUP BY uo.owner_uuid \n#pageable\n", 
			countQuery = "SELECT COUNT(*) FROM (SELECT su.full_name, su.username, su.email, su.phone, SUM(uo.total_package), SUM(uo.credit_amount) FROM user_outcome uo JOIN session_users su ON uo.owner_uuid = su.uuid WHERE uo.to_date BETWEEN ?1 AND ?2 AND su.full_name LIKE %?3% GROUP BY uo.owner_uuid) c", nativeQuery = true)
