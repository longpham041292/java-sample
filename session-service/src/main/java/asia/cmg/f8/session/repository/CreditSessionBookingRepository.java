...
public interface CreditSessionBookingRepository extends JpaRepository<CreditSessionBookingEntity, Long> {
	...
	String QUERY_GET_SESSIONS_HISTORY = "SELECT * " +
										"FROM credit_session_bookings " +
										"WHERE user_uuid = ?1 AND pt_uuid = ?2 AND status IN ?3 " +
										"ORDER BY id DESC " + PAGEABLE_QUERY;
	String COUNT_QUERY_GET_SESSIONS_HISTORY = "SELECT COUNT(*) " +
												"FROM credit_session_bookings " +
												"WHERE user_uuid = ?1 AND pt_uuid = ?2 AND status IN ?3";
	...
	String QUERY_GET_RECENT_TRAINERS = 	"SELECT sb.pt_uuid, su.username, su.full_name, su.avatar, su.`level`, lv.pt_booking_credit, MAX(sb.start_time) " +
										...
										"WHERE sb.user_uuid = :euUuid AND sb.status IN :status " +
										"GROUP BY sb.pt_uuid " +
										"ORDER BY sb.start_time DESC" + PAGEABLE_QUERY;
	String COUNT_QUERY_GET_RECENT_TRAINERS = 	
										"SELECT COUNT(DISTINCT sb.pt_uuid) " +
										"FROM credit_session_bookings sb JOIN session_users su ON sb.pt_uuid = su.uuid AND su.doc_status = 'approved' " +
										"WHERE sb.user_uuid = :euUuid AND sb.status IN :status " +
										"GROUP BY sb.pt_uuid";
	...
	String QUERY_GET_RECENT_CLIENTS = 	"SELECT sb.user_uuid, su.username, su.full_name, su.avatar, MAX(sb.start_time) " +
										...
										"WHERE sb.pt_uuid = :ptUuid AND sb.status IN :status " +
										"GROUP BY sb.user_uuid " +
										"ORDER BY sb.start_time DESC" + PAGEABLE_QUERY;
	String COUNT_QUERY_GET_RECENT_CLIENTS = 
										"SELECT COUNT(*) " +
										"FROM credit_session_bookings sb " +
										"WHERE sb.pt_uuid = :ptUuid AND sb.status IN :status " +
										"GROUP BY sb.user_uuid";

	List<CreditSessionBookingEntity> findByStartTimeBetweenAndUserUuidOrderByStartTime(LocalDateTime startTime, LocalDateTime endTime, String UserUuid);

	List<CreditSessionBookingEntity> findByStartTimeBetweenAndPtUuidOrderByStartTime(LocalDateTime startTime, LocalDateTime endTime, String ptUuid);

	List<CreditSessionBookingEntity> findByUserUuidAndStatusNotIn(String userUuid, List<CreditBookingSessionStatus> status);
	...
	@Query(value = QUERY_GET_RECENT_TRAINERS,
			countQuery = COUNT_QUERY_GET_RECENT_TRAINERS,
			nativeQuery = true)
	Page<Object[]> getRecentBookedTrainersByUserUuid(@Param("euUuid") String euUuid, @Param("status") List<Integer> status, Pageable pageable);

	@Query(value = QUERY_GET_RECENT_CLIENTS,
			countQuery = COUNT_QUERY_GET_RECENT_CLIENTS,
			nativeQuery = true)
	Page<Object[]> getRecentBookedClientsByTrainerUuid(@Param("ptUuid") String ptUuid, @Param("status") List<Integer> status, Pageable pageable);

	Optional<CreditSessionBookingEntity> findFirstByUuidAndPtUuid(final String uuid, final String ptUuid);
	
	Optional<CreditSessionBookingEntity> findFirstByUserUuidAndPtUuidAndCreditBookingId(final String userUuid, final String ptUuid, final long creditBookingId);
	...
	@Query(value = 
			...
			"WHERE cb.bookingType IN ?1 AND cb.status IN ?2 AND cb.endTime < NOW() ")
	List<CreditBookingEntity> getBurnedSessionsToDeduct(List<BookingServiceType> bookingType , List<CreditBookingSessionStatus> status);

	@Query(	value = QUERY_GET_SESSIONS_HISTORY,
			countQuery = COUNT_QUERY_GET_SESSIONS_HISTORY,
			nativeQuery = true)
	Page<CreditSessionBookingEntity> getSessionsHistory(String clientUuid, String trainerUuid, List<Integer> statusList, Pageable pageable);


	List<CreditSessionBookingEntity> findByStartTimeBetweenAndPtUuidAndStatusInOrderByStartTime(LocalDateTime startTime, LocalDateTime endTime, String ptUuid, List<CreditBookingSessionStatus> completedCode);


	@Query(value = "SELECT count(csb.uuid), sum(csb.credit_amount) "
	+ "FROM credit_session_bookings csb "
	+ "WHERE csb.status IN ?3 "
	+ "AND csb.created_date BETWEEN ?1 AND ?2" , nativeQuery = true)
	List<Object[]> countCancelledSessionsBookingByRange(LocalDateTime start, LocalDateTime end, List<Integer> cancelledCode);

	@Query(value = "SELECT csb.status, COUNT(csb.uuid) AS counting FROM credit_session_bookings csb "
			 + "WHERE csb.status IN ?3 "
			 + "AND csb.created_date BETWEEN ?1 AND ?2 "
	...
	@Query(value = "SELECT COUNT(DISTINCT (csb.booked_by)) "
	+ "FROM credit_session_bookings csb "
	+ "WHERE csb.created_date BETWEEN ?1 AND ?2 "
	+ "AND csb.pt_uuid = ?3" , nativeQuery = true)
	Integer getStatsNewClients(LocalDateTime start, LocalDateTime end, String ptUuid);

	@Query(value = "SELECT COUNT(DISTINCT (csb.uuid)), sum(csb.credit_amount)"
	+ "FROM credit_session_bookings csb "
	+ "WHERE csb.status IN ?4 "
	+ "AND csb.pt_uuid = ?3 "
	+ "AND csb.created_date BETWEEN ?1 AND ?2 " , nativeQuery = true)
	List<Object[]> getStatsTotalCompletedSession(LocalDateTime start, LocalDateTime end, String ptUuid, List<Integer> completedCode);

	@Query(value = "SELECT sum(csb.credit_amount) "
	+ "FROM credit_session_bookings csb "
	+ "WHERE csb.status IN ?4 "
	+ "AND csb.pt_uuid = ?3 "
	+ "AND csb.created_date BETWEEN ?1 AND ?2 " , nativeQuery = true)
	...
	@Query(value = "SELECT count(cb.id), sum(cb.credit_amount) "
	+ "FROM credit_bookings cb "
	+ "WHERE cb.status IN ?3 "
	+ "AND cb.created_date BETWEEN ?1 AND ?2 "
	+ "AND cb.booking_type = ?4" , nativeQuery = true)
	...
	List<BookingManagementDTO> sessionBookingManagement( 
